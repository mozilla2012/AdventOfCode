...
// Ripped this helper off Stack Overflow: https://stackoverflow.com/questions/39899072/how-can-i-find-the-prime-factors-of-an-integer-in-javascript
...
export function adventMain(input: string): any {
    ...
    for(let line of lines.slice(2)) { // set up map
    ...
    for(let i = 0; i < currentNode.length; i++) { // iterate through map.
        while (currentNode[i]!.charAt(2) !== 'Z') {
            for(let direction of directions.split('')) {                
                currentNode[i] = direction === 'L' ? map.get(currentNode[i]!)![0]! : map.get(currentNode[i]!)![1]!;
                loopCount[i]++;
            }   
        }
    }
    const factorSet = new Set<number>(); // Figure out LCM
